...
function preencherSelectMarcas() {
	fetch("/pw1/data/marcas.json").then(file => {
		file.text().then(dataString => {
			let dataObject = JSON.parse(dataString);
			const selectElement = document.getElementById('marca-select');
			const optionsHTML = dataObject.sort((a, b) => {
				if (a.text < b.text) {
					return -1;
				}
				if (a.text > b.text) {
...
function preencherSelectModels() {
	fetch("/pw1/data/modelos.json").then(file => {
		file.text().then(dataString => {
			let dataObject = JSON.parse(dataString);
			document.getElementById("marca-select").addEventListener("change", function () {
				...
				if (montadora && dataObject[montadora]) {
					dataObject[montadora].sort((a, b) => {
						if (a.text < b.text) {
							return -1;
						}
						if (a.text > b.text) {
...
function loremIpsunGenerator() {
	return fetch("/pw1/data/lorem_ipsum_dictionary.json").then(file => {
		return file.text().then(dataString => {
			let dataObject = JSON.parse(dataString);
			...
			for (var i = 0; i < paragraphLenght; i++) {
				...
				for (var j = 0; j < sentenceLength; j++) {
					...
					for (var k = 0; k < wordsLength; k++) {
						var word = dataObject[Math.floor(Math.random() * dataObject.length)];
						if (k == 0) {
...
function loadAboutUs() {
	const loremText = loremIpsunGenerator().then(loremText => {
